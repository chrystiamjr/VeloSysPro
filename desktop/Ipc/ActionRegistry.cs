...
{
    /// <summary>
    /// Deep module encapsulating type-safe Action payload parsing, mutation classification,
    /// and domain handler execution.
    /// </summary>
    public sealed class ActionRegistry
    {
        ...
        public bool IsMutation(string action) => _mutations.Contains(action);

        public bool Execute(string action, JsonElement payload)
        {
        ...
        private static bool MutateAndRefresh(Action mutation, Action refresh)
        {
            mutation();
            refresh();
            return true;
        ...
        public void PushRestorePoints() =>
            _events.EmitJson(IpcEvents.RestorePointsLoaded, _systemRestore.GetRestorePointsJson());

        public void PushSettings() => _events.Emit(IpcEvents.SettingsLoaded, _settings.Current);

        public void PushTweaks() => _events.EmitJson(IpcEvents.TweaksLoaded, _tweaks.GetTweaksJson());
        ...
        public void PushHistory() => _events.Emit(IpcEvents.HistoryLoaded, _tweaks.LoadHistory());
